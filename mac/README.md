...
Three things in the project are placeholders for your account:

1. **Bundle identifier** is `com.jefftk.WhistleSynth`.  It has to match an App
   ID registered to your team, with App Sandbox and Audio Input enabled on it.
2. **Signing** is set to Automatic with no team.  Pick your team in Signing &
   Capabilities; Xcode will fill in `DEVELOPMENT_TEAM`.  `build.sh` signs
   ad-hoc, which is not submittable -- the store wants Apple Distribution and
   a Mac App Store provisioning profile, which means an archive from Xcode.
3. **The icon** is generated by `mac/tools/make-icon.swift` -- a sine wave
...
And one thing that has never been done: **the `.xcodeproj` has never been
archived.**  `build.sh` is the build that gets run, so the project file is a
parallel truth that nobody has checked.  Do one `xcodebuild -scheme
WhistleSynth archive` before trusting it.  The Frameworks phase now links
AudioToolbox and CoreAudio explicitly rather than leaving the C files to clang
module autolinking, which is one fewer thing for that first archive to
discover.

On the App Store Connect side: a support URL, macOS screenshots, and the App
Privacy questionnaire, which can honestly be answered **Data Not Collected** --
that is what `PrivacyInfo.xcprivacy` says and what the usage string says.
